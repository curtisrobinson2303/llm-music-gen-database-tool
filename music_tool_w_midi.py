...
def chord_to_midi_notes(chord):
    ...
    # Use first one or two characters for the root
...
def convert_csv_to_midi(csv_file, output_folder="midi_files"):
    ...
    The CSV is expected to have 'Timestamp' and 'Chord' columns.
    ...
    ticks_per_beat = 480
    # Assume tempo of 120 BPM: 120 BPM = 2 beats per second, so 1 second = 960 ticks.
    ...
    for event in events:
        ...
        for note in notes:
            ...
            delta = 0  # subsequent events at same tick have 0 delta
        prev_tick = tick
...
def process_csv_folder(input_folder, output_folder="midi_files"):
    ...
    for csv_file in csv_files:
        ...
        convert_csv_to_midi(full_path)
    print("CSV to MIDI conversion complete.")

### MAIN FUNCTION WITH COMMAND-LINE INTERFACE ###
def main():
    parser = argparse.ArgumentParser(
        description="Music Tool: Convert MP3s to WAVs, extract chord progressions, visualize chords, or convert CSV to MIDI."
    )
